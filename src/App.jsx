...
async function registerFCMToken() {
  ...
  try { const t=await getToken(messaging,{vapidKey:VAPID_KEY}); if(t){myFcmToken=t;await setDoc(doc(db,"fcm_tokens",t),{token:t,createdAt:Date.now()});return t;} } catch(e) {}
  return null;
}
async function sendPushToAll(title,body,dedupeKey,winnerTeam,senderToken,category) {
  if(dedupeKey&&dedupeKey===lastPushKey)return; lastPushKey=dedupeKey;
  try{await fetch('/api/send-push',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,body,tag:dedupeKey||'ryder-push',winnerTeam:winnerTeam||null,senderToken:senderToken||null,category:category||'matchDecision'})});}catch(e){}
}
...
function calcTournament(days,numDays,matchesPerDay){
  ...
  const s=t1p+t2p;
  // Fix 4 (v3.04): proper win probability using binomial model.
  // If a team already reached the needed points → 100%.
  // Otherwise: for each remaining undecided round, assume 50/50.
  // P(win) = sum of C(remaining, k) * 0.5^remaining for k >= pointsStillNeeded.
  let t1wp=50,t2wp=50;
  if(t1c>=needed){t1wp=100;t2wp=0;}
  else if(t2c>=needed){t1wp=0;t2wp=100;}
  else{
    const remaining=totalPossible-t1c-t2c;
    if(remaining<=0){t1wp=t1c>t2c?100:t2c>t1c?0:50;t2wp=100-t1wp;}
    else{
      const t1needs=Math.ceil(needed-t1c);
      // Binomial CDF: P(X >= t1needs) where X ~ Bin(remaining, 0.5)
      const choose=(n,k)=>{if(k<0||k>n)return 0;let r=1;for(let i=0;i<k;i++)r=r*(n-i)/(i+1);return r;};
      let prob=0;
      for(let k=t1needs;k<=remaining;k++)prob+=choose(remaining,k);
      prob=prob/Math.pow(2,remaining);
      t1wp=Math.max(1,Math.min(99,Math.round(prob*100)));
      t2wp=100-t1wp;
    }
  }
  return{t1Confirmed:t1c,t2Confirmed:t2c,t1Projected:Math.round(t1p*10)/10,t2Projected:Math.round(t2p*10)/10,t1WinProb:t1wp,t2WinProb:t2wp,totalPoints:totalPossible,needed};
}
...
function buildTournamentWinMessage(winner,t1Pts,t2Pts,t1Name,t2Name){
  ...
  const dot=winner===t1Name?"🔵":"🔴";
  const margin=wPts-lPts;
  return{
    title:`🏆 ${dot} ${winner} gewinnt den Ryder Cup!`,
...
const FAN_TEAM_KEY="ryder_fan_team";
const DEFAULT_PREFS={matchDecision:true,leadChange:false,everyMessage:false};
...
function useChatPrefs(){
  ...
  const saveToFirestore=async()=>{
    setSaveState("saving");
    try{
      if(myFcmToken){await setDoc(doc(db,"fcm_tokens",myFcmToken),{token:myFcmToken,createdAt:Date.now(),prefs},{ merge:true });}
      localStorage.setItem(CHAT_PREFS_KEY,JSON.stringify(prefs));
    }catch(e){}
    setSaveState("done");setTimeout(()=>setSaveState("idle"),1800);
  };
  return[prefs,update,saveToFirestore,saveState];
}
...
function ChatProfileSetup({role,T,onDone,matchPlayers,existingProfile}){
  const isPlayer=role&&role.startsWith("match-");
  const roleLabel=role==="admin"?"Admin":role==="viewer"?"Zuschauer":`Spieler ${parseInt(role.split("-")[1])+1}`;
  const [name,setName]=useState(existingProfile?existingProfile.name:roleLabel);
  const [photo,setPhoto]=useState(existingProfile?existingProfile.photo:null);
  const [pickerDone,setPickerDone]=useState(!isPlayer||!matchPlayers||!!existingProfile);
  ...
  // v3.15: Player picker — select which player you are from the match
  if(!pickerDone&&isPlayer&&matchPlayers&&matchPlayers.length>0){
    return(
      <div style={{padding:"24px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:"16px"}}>
        <div style={{fontSize:"16px",fontWeight:"700",color:T.gold,textAlign:"center"}}>⛳ Welcher Spieler bist du?</div>
        <div style={{fontSize:"12px",color:T.muted,textAlign:"center"}}>Tippe auf deinen Namen um dem Chat beizutreten</div>
        <div style={{width:"100%",maxWidth:"320px",display:"flex",flexDirection:"column",gap:"8px"}}>
          {matchPlayers.map((p,i)=>{
            ...
            return(
              <button key={i} onClick={()=>{setName(p.name);setPhoto(p.photo);setPickerDone(true);}}
                style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 14px",background:T.surface,border:`1.5px solid ${color}33`,borderRadius:"10px",cursor:"pointer",textAlign:"left"}}>
                {p.photo
                  ?<img src={p.photo} style={{width:"44px",height:"44px",borderRadius:"50%",objectFit:"cover",border:`2px solid ${color}`}} alt=""/>
                  :<div style={{width:"44px",height:"44px",borderRadius:"50%",background:color+"22",border:`2px solid ${color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",fontWeight:"700",color}}>{(p.name||"?").split(" ").map(w=>w[0]).join("").slice(0,2)}</div>}
                <div style={{flex:1}}>
                  <div style={{fontSize:"14px",fontWeight:"600",color:T.cream}}>{p.name}</div>
                  <div style={{fontSize:"10px",color}}>{p.team==="t1"?"Team Europa":"Team USA"}</div>
                </div>
                <div style={{fontSize:"18px",color:T.faint}}>›</div>
              </button>
            );
          })}
        </div>
...
// ── Chat Bubble ───────────────────────────────────────────────────────────────
function ChatBubble({msg,myUid,isAdmin,T,onReact,onDelete,onReply,onScrollToMsg,allMsgs}){
  ...
  const typeColor={matchDecision:T.gold,matchDecisionT1:T.blue,matchDecisionT2:T.red,birdie:T.blue,leadChange:T.gold,halftime:T.muted,equalize:T.muted,everyHole:T.faint,dayEnd:T.gold};
  const typeBg={matchDecision:T.gold+"18",matchDecisionT1:T.blue+"18",matchDecisionT2:T.red+"18",birdie:T.blue+"18",leadChange:T.gold+"18",halftime:T.elevated,equalize:T.elevated,everyHole:T.elevated,dayEnd:T.gold+"18"};
  ...
  const ReactionBar=()=>(
    <div style={{display:"flex",gap:"4px",marginTop:"4px",flexWrap:"wrap",paddingLeft:"4px"}}>
      ...
      {onReply&&<button onClick={()=>onReply(msg)} style={{background:"transparent",border:`1px solid ${T.border}44`,borderRadius:"12px",padding:"2px 7px",fontSize:"10px",cursor:"pointer",color:T.faint}}>↩</button>}
      {(isAdmin||(msg.authorId&&msg.authorId===myUid))&&onDelete&&<button onClick={()=>onDelete(msg.id)} style={{background:"transparent",border:`1px solid #E0525233`,borderRadius:"12px",padding:"2px 7px",fontSize:"9px",cursor:"pointer",color:"#E05252",marginLeft:"auto"}}>🗑</button>}
    </div>
  );
        ...
        <div style={{background:bg,border:`1px solid ${border}44`,borderLeft:`3px solid ${border}`,borderRadius:"0 8px 8px 0",padding:"7px 10px"}}>
          {photos.length>0&&(
            <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
              {photos.map((p,i)=>(<img key={i} src={p} style={{width:"28px",height:"28px",borderRadius:"50%",objectFit:"cover",border:`2px solid ${border}`}} alt=""/>))}
            </div>
          )}
          <div style={{fontSize:"11px",color:T.cream,lineHeight:1.4,whiteSpace:"pre-line"}}>{msg.text}</div>
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:isMe?"flex-end":"flex-start",marginBottom:"8px"}}>
      {!isMe&&(
        <div style={{display:"flex",alignItems:"center",gap:"5px",marginBottom:"3px",paddingLeft:"4px"}}>
          ...
          <div style={{fontSize:"9px",color:T.faint}}>{msg.author}{msg.fanTeam&&<span style={{marginLeft:"4px"}}>{msg.fanTeam==="t1"?"🔵":"🔴"}</span>}</div>
        </div>
      )}
      <div style={{maxWidth:"80%"}}>
        ...
        {isPhoto&&msg.imageUrl&&(
          <div style={{borderRadius:isMe?"12px 12px 2px 12px":"12px 12px 12px 2px",overflow:"hidden",border:`2px solid ${isMe?T.gold+"55":T.border}`,cursor:"pointer"}} onClick={()=>setImgExpanded(true)}>
            {msg.replyToId&&(()=>{
              const orig=(allMsgs||[]).find(m=>m.id===msg.replyToId);
              return(
                <div onClick={e=>{e.stopPropagation();onScrollToMsg&&onScrollToMsg(msg.replyToId);}} style={{background:isMe?T.gold:T.elevated,borderBottom:`1px solid ${T.border}`,padding:"4px 8px",fontSize:"10px",color:T.faint,lineHeight:1.3}}>
                  <div style={{fontWeight:700,color:T.gold,marginBottom:1}}>{msg.replyToAuthor||"..."}</div>
                  <div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"200px"}}>{orig?orig.text||"📸 Foto":msg.replyToText||"..."}</div>
                </div>
              );
            })()}
        ...
        {!isPhoto&&(
          <div style={{background:isMe?T.gold:T.elevated,border:`1px solid ${isMe?T.gold+"55":T.border}`,borderRadius:isMe?"12px 12px 2px 12px":"12px 12px 12px 2px",padding:"8px 10px"}}>
            {msg.replyToId&&(()=>{
              const orig=(allMsgs||[]).find(m=>m.id===msg.replyToId);
              return(
                <div onClick={()=>onScrollToMsg&&onScrollToMsg(msg.replyToId)} style={{background:isMe?"rgba(0,0,0,0.12)":"rgba(255,255,255,0.06)",borderLeft:`3px solid ${T.gold}`,borderRadius:"0 4px 4px 0",padding:"4px 8px",marginBottom:"6px",cursor:"pointer",fontSize:"10px",color:T.faint,lineHeight:1.3}}>
                  <div style={{fontWeight:700,color:T.gold,marginBottom:1}}>{msg.replyToAuthor||"..."}</div>
                  <div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"200px"}}>{orig?orig.text||"📸 Foto":msg.replyToText||"..."}</div>
                </div>
              );
...
function NotifSettings({prefs,onUpdate,onSave,saveState,T}){
  const rows=[
    {key:"matchDecision",label:"Match-Entscheidungen",sub:"Runde gewonnen/verloren, Spieltag-Ende, Turniersieg"},
    {key:"leadChange",label:"Ausgleich / Lead-Wechsel",sub:"Führungswechsel und All Square"},
    {key:"everyMessage",label:"Jede Nachricht",sub:"Alle Chat-Events anzeigen"},
  ];
  return(
    <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"12px 14px",marginBottom:"10px"}}>
      <div style={{fontSize:"10px",color:T.gold,letterSpacing:"1px",fontWeight:"700",marginBottom:"4px",display:"flex",alignItems:"center",gap:"6px"}}><IconBell size={12} color={T.gold}/>PUSH-BENACHRICHTIGUNGEN</div>
      <div style={{fontSize:"9px",color:T.faint,marginBottom:"10px"}}>Steuert nur Push auf dem Home Screen — im Chat werden immer alle Events angezeigt.</div>
      {rows.map(r=>(
      ...
      {onSave&&(
        <button onClick={onSave} disabled={saveState==="saving"} style={{width:"100%",marginTop:"12px",padding:"10px",background:saveState==="done"?"#4CAF50":saveState==="saving"?T.elevated:`linear-gradient(135deg,${T.gold},#A07830)`,border:"none",borderRadius:"8px",color:saveState==="done"?"white":T.isDark?"#0D2B1A":"white",fontSize:"12px",fontWeight:"700",cursor:"pointer",transition:"all 0.3s",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}>
          {saveState==="done"?"✓ Gespeichert":saveState==="saving"?"Speichere...":"Speichern"}
        </button>
      )}
    </div>
  );
...
function useFanVotes(){
  const [votes,setVotes]=useState({t1:0,t2:0});
  useEffect(()=>{
    let unsubscribe=null;
    try{
      const q=collection(db,"fanVotes");
      unsubscribe=onSnapshot(q,
        snap=>{try{let t1=0,t2=0;snap.forEach(d=>{const v=d.data();if(v&&v.team==="t1")t1++;else if(v&&v.team==="t2")t2++;});setVotes({t1,t2});}catch(e){}},
...
function LiveChat({role,T,mini=true,onExpand,onClearChat,matchPlayers}){
  const fanTeam=typeof window!=="undefined"?localStorage.getItem(FAN_TEAM_KEY):null;
  const msgs=useChatMessages();
  const [prefs,updatePrefs,savePrefsToFirestore,prefsSaveState]=useChatPrefs();
  ...
  // v3.09: show ALL messages in chat always — prefs only control push notifications
  const visible=msgs;
  ...
  const fanVotesData=useFanVotes();
  ...
  const send=async()=>{
    ...
    const msgData={type:"user",author:p.name,photo:p.photo||null,authorId:myUid,text:text.trim(),fanTeam:fanTeam||null};
    if(replyTo){msgData.replyToId=replyTo.id;msgData.replyToAuthor=replyTo.author;msgData.replyToText=(replyTo.text||"").slice(0,80);}
    await postChatMessage(msgData);
    // v3.11: push notification for user messages — include senderUid so own device can skip
    try{await sendPushToAll(`💬 ${p.name}`,text.trim().slice(0,100),`chat-${Date.now()}`,null,myFcmToken,'everyMessage');}catch(e){}
    setText("");setReplyTo(null);
  };

  const isPlayer=role&&role.startsWith("match-");
  const canSendPhoto=isAdmin||isPlayer;

  const sendPhoto=async(file)=>{
    if(!file||!canSendPhoto)return;
    ...
    try{
      ...
      // v3.07: Resize to max 800px + quality 0.7 to stay well under Firestore 1MB doc limit
      const MAX=800;
      ...
      if(imageUrl.length>900000){
        alert("Bild ist zu groß. Bitte ein kleineres Foto verwenden.");
        setPhotoSending(false);return;
      }
      await postChatMessage({type:"photo",author:p.name,photo:p.photo||null,authorId:myUid,imageUrl,text:"",ts:Date.now()});
      // v3.11: push for photo — include senderUid
      try{await sendPushToAll(`📸 ${p.name}`,"Hat ein Foto geteilt",`photo-${Date.now()}`,null,myFcmToken,'everyMessage');}catch(e){}
    }catch(e){console.error("Photo send failed",e);alert("Foto konnte nicht gesendet werden.");}
  ...
  if(!mini&&!profile&&!showProfileEdit){
    return <ChatProfileSetup role={role} T={T} matchPlayers={matchPlayers} onDone={p=>{saveProfile(p);}}/>;
  }
  if(!mini&&showProfileEdit){
    return(
      <div>
        <ChatProfileSetup role={role} T={T} matchPlayers={matchPlayers} existingProfile={profile} onDone={p=>{saveProfile(p);setShowProfileEdit(false);}}/>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"8px",height:"calc(100vh - 132px)"}}>
      ...
      {/* v3.13: Fan barometer */}
      {!mini&&!showSettings&&fanVotesData&&(fanVotesData.t1+fanVotesData.t2)>0&&(
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"8px",padding:"8px 12px",marginBottom:"6px",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:"9px",color:T.faint,marginBottom:"4px"}}>
            <span style={{color:T.blue}}>🔵 {fanVotesData.t1} Fans</span>
            <span style={{fontWeight:"700",color:T.gold}}>Fan-Barometer</span>
            <span style={{color:T.red}}>🔴 {fanVotesData.t2} Fans</span>
          </div>
          <div style={{display:"flex",height:"8px",borderRadius:"4px",overflow:"hidden",background:T.elevated}}>
            <div style={{width:Math.round(fanVotesData.t1/(fanVotesData.t1+fanVotesData.t2)*100)+"%",background:T.blue,transition:"width 1s cubic-bezier(0.4,0,0.2,1)",borderRadius:"4px 0 0 4px"}}/>
            <div style={{width:Math.round(fanVotesData.t2/(fanVotesData.t1+fanVotesData.t2)*100)+"%",background:T.red,transition:"width 1s cubic-bezier(0.4,0,0.2,1)",borderRadius:"0 4px 4px 0"}}/>
          </div>
        </div>
      )}

      {showSettings&&<NotifSettings prefs={prefs} onUpdate={updatePrefs} onSave={savePrefsToFirestore} saveState={prefsSaveState} T={T}/>}

      <div ref={scrollRef} onScroll={handleScroll} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"10px 12px",flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch",display:"flex",flexDirection:"column",justifyContent:visible.length<6?"flex-end":"flex-start",position:"relative"}}>
        {visible.length===0&&(
        ...
        {visible.map(m=><div key={m.id} id={"msg-"+m.id}><ChatBubble msg={m} myUid={myUid} isAdmin={isAdmin} T={T} onReact={react} onDelete={deleteMsg} onReply={(msg)=>setReplyTo({id:msg.id,author:msg.author||"System",text:msg.text||""})} onScrollToMsg={scrollToMsg} allMsgs={visible}/></div>)}
      </div>

      {/* v3.08: Scroll-to-bottom button */}
      {showScrollBtn&&(
        <div style={{display:"flex",justifyContent:"center",marginTop:"-20px",marginBottom:"6px",position:"relative",zIndex:5}}>
          <button onClick={scrollToBottom} style={{background:T.gold,border:"none",borderRadius:"50%",width:"32px",height:"32px",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.3)",fontSize:"14px",color:T.isDark?"#0D2B1A":"white"}}>↓</button>
        </div>
      )}

      {/* v3.08: Reply preview above input */}
      {replyTo&&(
        <div style={{display:"flex",alignItems:"center",gap:"8px",background:T.elevated,border:`1px solid ${T.gold}44`,borderLeft:`3px solid ${T.gold}`,borderRadius:"0 6px 6px 0",padding:"6px 10px",marginBottom:"6px"}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:"9px",color:T.gold,fontWeight:700}}>↩ Antwort an {replyTo.author}</div>
            <div style={{fontSize:"11px",color:T.faint,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{replyTo.text||"📸 Foto"}</div>
          </div>
          <button onClick={()=>setReplyTo(null)} style={{background:"transparent",border:"none",color:T.faint,fontSize:"16px",cursor:"pointer",padding:"0 4px"}}>✕</button>
        </div>
      )}
      ...
      <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
        ...
        <textarea value={text} onChange={e=>{setText(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,100)+"px";}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder={`Als ${avatarName}...`} rows={1} style={{flex:1,minWidth:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:"8px",color:T.cream,fontSize:"13px",padding:"8px 10px",outline:"none",resize:"none",maxHeight:"100px",lineHeight:"1.4",fontFamily:"inherit"}}/>
        {/* Photo buttons — only for admin + players */}
        {canSendPhoto&&(<>
...
function Login({onLogin,theme,onThemeToggle}){
  const [mode,setMode]=useState(null); // null | "spieler"
      ...
      <div style={{flex:1,padding:"20px 20px 40px",maxWidth:"400px",width:"100%",margin:"0 auto",display:"flex",flexDirection:"column",justifyContent:"center"}}>
        {!mode?(
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            <button onClick={()=>onLogin("viewer")} style={{width:"100%",padding:"16px",background:"#0F2D1A",border:"1.5px solid #2D6B40",borderRadius:"10px",color:"#F2EDD7",fontSize:"15px",fontWeight:"700",letterSpacing:"2px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"10px"}}>
              👁 Zuschauer
            </button>
            <button onClick={()=>setMode("spieler")} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#C9A84C,#A07830)",border:"none",borderRadius:"10px",color:"#071A0E",fontSize:"15px",fontWeight:"900",letterSpacing:"2px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"10px"}}>
              ⛳ Spieler / Admin
            </button>
          </div>
        ):(
          <div>
            <button onClick={()=>{setMode(null);setCode("");setError("");}} style={{background:"transparent",border:"none",color:"#8BAF7C",fontSize:"12px",cursor:"pointer",marginBottom:"12px",display:"flex",alignItems:"center",gap:"4px"}}>← Zurück</button>
            <input
              ...
              placeholder="CODE EINGEBEN" value={code} autoFocus
              onChange={e=>{setCode(e.target.value.toUpperCase());setError("");}}
              onKeyDown={e=>e.key==="Enter"&&check()}/>
            {error&&<div style={{color:"#E05252",fontSize:"12px",marginBottom:"10px",textAlign:"center"}}>{error}</div>}
            <button
              style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#C9A84C,#A07830)",border:"none",borderRadius:"8px",color:"#071A0E",fontSize:"14px",fontWeight:"900",letterSpacing:"3px",textTransform:"uppercase",cursor:"pointer"}}
              onClick={check}>Eintreten</button>
            <div style={{fontSize:"11px",color:"#8BAF7C",textAlign:"center",marginTop:"12px"}}>Gib deinen Code ein</div>
          </div>
        )}
        <div style={{textAlign:"center",fontSize:"10px",color:"#2D6B40",marginTop:"16px",letterSpacing:"1px"}}>v{VERSION}</div>
      </div>
    </div>
  );
}
...
function ScoreModal({match,holeIndex,t1Name,t2Name,existing,par,onSave,onClose,T}){
  // Fix 2 (v3.04): swipe-down-to-close
  const [dragY,setDragY]=useState(0);
  const dragStartRef=useRef(null);
  const handleTouchStart=e=>{dragStartRef.current=e.touches[0].clientY;};
  const handleTouchMove=e=>{if(dragStartRef.current===null)return;const dy=e.touches[0].clientY-dragStartRef.current;setDragY(Math.max(0,dy));};
  const handleTouchEnd=()=>{if(dragY>100)onClose();else setDragY(0);dragStartRef.current=null;};
  ...
  return(
    <div style={{position:"fixed",inset:0,background:`rgba(0,0,0,${Math.max(0.2,0.8-dragY/300)})`,display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:100}} onClick={onClose}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"16px 16px 0 0",padding:"12px 16px 20px",width:"100%",maxWidth:"480px",maxHeight:"94vh",overflowY:dragY>0?"hidden":"auto",WebkitOverflowScrolling:"touch",transform:`translateY(${dragY}px)`,transition:dragY===0?"transform 0.25s ease":"none"}} onClick={e=>e.stopPropagation()}>
        <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{cursor:"grab",padding:"8px 0 4px",touchAction:"none"}}>
        ...
        {isScramble&&(
          <div style={{display:"flex",flexDirection:"column",gap:"12px",marginBottom:"14px"}}>
            ...
            <div style={{background:T.red+"11",borderRadius:"12px",border:`1px solid ${T.red}33`,padding:"10px 12px"}}>
              ...
              {/* Fix 1 (v3.04): use same component as t1 for consistent layout */}
              {t2ShotsBlock}
            </div>
          </div>
        )}
...
function MatchCard({match,pars,t1Name,t2Name,canEdit,isAdmin,isViewer,dayDone,isOwnMatch,onHoleClick,onReset,T,captainT1,captainT2}){
  ...
  return(
    <>
      <div style={{background:T.surface,border:`2px solid ${canEdit?T.gold:isOwnMatch&&dayDone?"#4CAF50":T.border}`,borderRadius:"12px",marginBottom:"12px",overflow:"hidden",opacity:canEdit||isAdmin||isViewer||dayDone?1:0.45,boxShadow:canEdit?`0 0 0 1px ${T.gold}33,0 4px 20px ${T.gold}18`:isOwnMatch&&dayDone?`0 0 0 1px #4CAF5033`:"none"}}>
        <div style={{padding:"10px 14px",background:canEdit?T.gold+"11":T.isDark?"#0A2014":T.elevated,borderBottom:`1px solid ${T.border}`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
              <div style={{fontSize:"13px",fontWeight:"900",color:canEdit?T.gold:isOwnMatch&&dayDone?"#4CAF50":T.cream}}>{match.name}</div>
              ...
              {!canEdit&&isOwnMatch&&dayDone&&<div style={{background:"#4CAF50",color:"white",fontSize:"9px",fontWeight:"900",borderRadius:"4px",padding:"2px 8px"}}>⛳ DEIN MATCH</div>}
        ...
        <div style={{padding:"10px 14px"}}>
          ...
          {!canEdit&&!isAdmin&&!isViewer&&!dayDone&&<div style={{marginTop:"8px",fontSize:"10px",color:T.faint,textAlign:"center"}}>🔒 Nur lesbar</div>}
        </div>
      </div>
...
function HoleHeatmap({holeWins,T,t1Name,t2Name}){
  // v3.06: Variant B — large grid cells with proportional vertical fill + summary
  const CELL_H=56;
  const allHoles=Array.from({length:18},(_,i)=>({hi:i,t1:holeWins.t1[i]||0,t2:holeWins.t2[i]||0,tie:holeWins.tie[i]||0}));
  const totEU=allHoles.reduce((a,h)=>a+h.t1,0);
  const totUS=allHoles.reduce((a,h)=>a+h.t2,0);
  const totTie=allHoles.reduce((a,h)=>a+h.tie,0);
  const totAll=totEU+totUS+totTie;
  return(
    <div>
      {[0,1].map(r=>{
        const holes=allHoles.slice(r*9,r*9+9);
        return(
          <div key={r} style={{marginBottom:14}}>
            <div style={{fontSize:9,color:T.gold,letterSpacing:1,marginBottom:6,fontWeight:700}}>{"Runde "+(r+1)+" · Loch "+(r*9+1)+"–"+(r*9+9)}</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",gap:4}}>
              {holes.map(h=>{
                const total=h.t1+h.t2+h.tie;
                if(total===0)return(<div key={h.hi} style={{height:CELL_H,background:T.elevated,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:T.faint}}>·</div>);
                return(
                  <div key={h.hi} style={{display:"flex",flexDirection:"column",borderRadius:6,overflow:"hidden",height:CELL_H,border:`1px solid ${T.border}`}}>
                    {h.t1>0&&<div style={{flex:h.t1,background:T.blue,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,color:"#fff",fontWeight:700}}>{h.t1}</span></div>}
                    {h.tie>0&&<div style={{flex:h.tie,background:T.isDark?"#2A3A2A":"#D8D8D8",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:10,color:T.isDark?"#888":"#777",fontWeight:500}}>{h.tie}</span></div>}
                    {h.t2>0&&<div style={{flex:h.t2,background:T.red,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,color:"#fff",fontWeight:700}}>{h.t2}</span></div>}
                  </div>
                );
              })}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(9,1fr)",gap:4,marginTop:3}}>
              {holes.map(h=><div key={h.hi} style={{textAlign:"center",fontSize:10,color:T.muted,fontWeight:500}}>{h.hi+1}</div>)}
            </div>
          </div>
        );
      })}
      {/* Summary bar + counts */}
      {totAll>0&&(
        <div style={{marginTop:14,padding:"12px 14px",background:T.elevated,borderRadius:8}}>
          <div style={{display:"flex",height:14,borderRadius:7,overflow:"hidden",marginBottom:10}}>
            {totEU>0&&<div style={{flex:totEU,background:T.blue}}/>}
            {totTie>0&&<div style={{flex:totTie,background:T.isDark?"#2A3A2A":"#D8D8D8"}}/>}
            {totUS>0&&<div style={{flex:totUS,background:T.red}}/>}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:900,color:T.blue,fontFamily:"'Arial Black',sans-serif",lineHeight:1}}>{totEU}</div>
              <div style={{fontSize:9,color:T.blue,marginTop:2}}>{t1Name||"Europa"}</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:16,fontWeight:700,color:T.muted,lineHeight:1}}>{totTie}</div>
              <div style={{fontSize:9,color:T.faint,marginTop:2}}>Geteilt</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:900,color:T.red,fontFamily:"'Arial Black',sans-serif",lineHeight:1}}>{totUS}</div>
              <div style={{fontSize:9,color:T.red,marginTop:2}}>{t2Name||"USA"}</div>
            </div>
          </div>
        </div>
      )}
...
function SpielerTab({days,t1Name,t2Name,T,allPlayers}){
  const [contribFilter,setContribFilter]=useState("shots"); // shots|abschlaege|fairways|putts
  const playerStats={};
  ...
  // Fix 1 (v3.05): Contribution leaderboard with filter at TOP
  const filterLabels={shots:"Gesamt",abschlaege:"Abschlag",fairways:"Fairway",putts:"Putts"};
  const filterIcons={shots:"⛳",abschlaege:"🏌️",fairways:"🟢",putts:"⛳"};
  const sorted=entries.slice().sort((a,b)=>b[contribFilter]-a[contribFilter]);
  const maxVal=Math.max(...sorted.map(p=>p[contribFilter]),1);

  return(
    <div>
      {/* ── CONTRIBUTION LEADERBOARD ── */}
      <ST>🏆 CONTRIBUTION LEADERBOARD</ST>
      <div style={{display:"flex",gap:5,marginBottom:12}}>
        {["shots","abschlaege","fairways","putts"].map(k=>(
          <button key={k} onClick={()=>setContribFilter(k)}
            style={{flex:1,padding:"7px 4px",borderRadius:8,border:contribFilter===k?`2px solid ${T.gold}`:`1px solid ${T.border}`,background:contribFilter===k?T.gold+"18":"transparent",color:contribFilter===k?T.gold:T.muted,fontSize:10,fontWeight:contribFilter===k?700:400,cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
            {filterLabels[k]}
          </button>
        ))}
      </div>
      <Card>
        {sorted.map((p,i)=>{
          const color=p.team==="t1"?T.blue:T.red;
          const val=p[contribFilter];
          const w=maxVal>0?Math.round((val/maxVal)*100):0;
          return(
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:9,marginBottom:i<sorted.length-1?13:0}}>
              <div style={{fontSize:13,fontWeight:700,color:T.faint,width:16,textAlign:"center",flexShrink:0}}>{i+1}</div>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{height:6,background:T.elevated,borderRadius:4,overflow:"hidden"}}>
              ...
              <Card key={p.id}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:10,color:T.faint}}>{p.holesLed} Loch angeführt · {p.birdieHoles} Birdies</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:22,fontWeight:900,color,fontFamily:"'Arial Black',sans-serif",lineHeight:1}}>{p.shots}</div>
                    <div style={{fontSize:9,color:T.faint}}>Schläge</div>
                  </div>
                </div>
                <div style={{display:"flex",gap:6}}>
                  {[{label:"🏌️ Abschl.",v:p.abschlaege,c:color},{label:"🟢 Fairway",v:p.fairways,c:"#4CAF50"},{label:"⛳ Putts",v:p.putts,c:"#2E7D32"},{label:"🐦 Birdie",v:p.birdieHoles,c:"#4CAF50"}].map((b,j)=>(
                    <div key={j} style={{flex:1,background:T.elevated,borderRadius:6,padding:4,textAlign:"center"}}>
                      <div style={{fontSize:10,color:b.c,fontWeight:700}}>{b.v}</div>
                      <div style={{fontSize:7,color:T.faint,lineHeight:1.3}}>{b.label}</div>
                    </div>
                  ))}
                </div>
              </Card>
...
function StatsTab({days,t1Name,t2Name,T,captainT1,captainT2,allPlayers}){
  ...
  const SECTIONS=["ÜBERSICHT","PAARE","LÖCHER","🏌️ SPIELER"];
  const Card=({children,style})=>(<div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"12px 14px",marginBottom:10,...style}}>{children}</div>);
  const ST=({children})=>(<div style={{fontSize:10,color:T.gold,letterSpacing:2,fontWeight:700,margin:"16px 0 10px",display:"flex",alignItems:"center",gap:6}}><div style={{flex:1,height:1,background:T.border}}/>{children}<div style={{flex:1,height:1,background:T.border}}/></div>);
  ...
  return(
    <div style={{paddingBottom:20}}>
      ...
      {sec===2&&(<>
        ...
        <Card>
          <HoleHeatmap holeWins={stats.holeWins} T={T} t1Name={t1Name} t2Name={t2Name}/>
        </Card>
      ...
      {sec===3&&<SpielerTab days={filteredDays} t1Name={t1Name} t2Name={t2Name} T={T} allPlayers={allPlayers}/>}
    </div>
  );
} 
...
function Dashboard({config,role,onBack,backLabel,onEndTournament,theme,onThemeToggle,onRefreshConfig}){
  const T=THEMES[theme];
  const {t1Name,t2Name,days}=config;
  const [modal,setModal]=useState(null);
  // v3.13: check URL param or day status for initial tab
  const [activeTab,setActiveTab]=useState(()=>{
    if(typeof window!=="undefined"){
      const params=new URLSearchParams(window.location.search);
      if(params.get("tab")==="chat"){window.history.replaceState({},"","/");return"chat";}
    }
    const d=config.days||[];
    if(d.length>=2&&d[0]?.status==="done"&&d[1]?.status==="active")return"day1";
    return"day0";
  });
  // v3.13: Listen for SW messages to open chat tab
  useEffect(()=>{
    const handler=e=>{if(e.data&&e.data.type==="openChat")setActiveTab("chat");};
    if(typeof navigator!=="undefined"&&navigator.serviceWorker){navigator.serviceWorker.addEventListener("message",handler);}
    return()=>{if(typeof navigator!=="undefined"&&navigator.serviceWorker){navigator.serviceWorker.removeEventListener("message",handler);}};
  },[]);
  const [saving,setSaving]=useState(false);
  ...
  const endDay=async(dayId)=>{
    ...
    // v3.07: post day-end chat message with score
    const dayNum=days.findIndex(d=>d.id===dayId)+1;
    const numDays=config.numDays||(config.daySettings&&config.daySettings.length)||days.length;
    const mPerDay=config.matchesPerDay||(days[0]&&days[0].matches&&days[0].matches.length)||4;
    const st=calcTournament(nd,numDays,mPerDay);
    const isLast=dayNum>=numDays;
    const dayText=isLast
      ?`🏁 Letzter Spieltag beendet!\nEndstand: ${t1Name} ${st.t1Confirmed} – ${st.t2Confirmed} ${t2Name}`
      :`📊 Spieltag ${dayNum} beendet\nZwischenstand: ${t1Name} ${st.t1Confirmed} – ${st.t2Confirmed} ${t2Name}`;
    await postChatMessage({type:"system",subtype:"dayEnd",text:dayText,ts:Date.now()},`dayEnd-${dayId}`);
    await saveTournament({...config,days:nd});setSaving(false);
  };
  ...
  const myPushUid=`${role}_${typeof window!=="undefined"?(localStorage.getItem("ryder_uid")||"anon"):"anon"}`;
  useEffect(()=>{if(!messaging)return;const u=onMessage(messaging,p=>{const title=p.data?.title||"";if(title.indexOf("🏆")!==-1||title.indexOf("gewinn")!==-1)return;const sender=p.data?.senderUid||"";if(sender&&sender===myPushUid)return;setToast({title,body:p.data?.body});});return()=>u();},[]);
  ...
  const saveScore=async(dayId,matchId,hi,t1s,t2s,extraScores)=>{
    ...
    const change=detectPointChange(days,nd,t1Name,t2Name);if(change)await sendPushToAll(change.title,change.body,change.key,change.winnerTeam,null,'matchDecision');
    ...
    if(match&&course){
      ...
      if(best<=par-2){
        const isEagle=best<=par-3;
        const label=isEagle?"🦅 EAGLE!":"🐦 Birdie!";
        const team1Better=t1s<=t2s;
        const teamDot=team1Better?"🔵":"🔴";
        await postChatMessage({type:"system",subtype:"birdie",text:`${label} ${teamDot} ${best} auf Loch ${holeNum} (Par ${par}) · ${matchName}`,ts:Date.now()},`birdie-${matchId}-${hi}`);
      }
      ...
      if(rs.won&&!prevRs?.won){
        // v3.07: richer chat messages with winner names, opponent names + photos
        ...
        const winPair=rs.diff>0?(match.t1Pair||[]):(match.t2Pair||[]);
        const losePair=rs.diff>0?(match.t2Pair||[]):(match.t1Pair||[]);
        const winIds=rs.diff>0?(match.t1PlayerIds||[]):(match.t2PlayerIds||[]);
        const winPhotos=rs.diff>0?(match.t1Photos||{}):(match.t2Photos||{});
        const winNames=winPair.join(" & ")||winner;
        const loseNames=losePair.join(" & ")||"";
        const dot=rs.diff>0?"🔵":"🔴";
        const rLabel="Runde "+(roundIdx+1);
        let text;
        if(winner){
          const phrasing=["holen den Punkt!","gewinnen!","setzen sich durch!","triumphieren!"];
          const pick=phrasing[Math.floor(Date.now()/1000)%phrasing.length];
          text=`🏆 ${dot} ${winNames} ${pick}\n${matchName} · ${rLabel} · ${rs.label}${loseNames?"\nvs. "+loseNames:""}`;
        }else{
          const p1=(match.t1Pair||[]).join(" & ");
          const p2=(match.t2Pair||[]).join(" & ");
          text=`🤝 ${matchName} · ${rLabel} endet All Square\n${p1} vs. ${p2} — Punkt wird geteilt`;
        }
        const photos=winIds.map(id=>winPhotos[id]).filter(Boolean);
        await postChatMessage({type:"system",subtype,text,winnerPhotos:photos,ts:Date.now()},`decision-${matchId}-${roundIdx}`);
      }
      ...
      if((prevRoundDiff>0&&newRoundDiff<0)||(prevRoundDiff<0&&newRoundDiff>0)){
        ...
        const leaderDot=newRoundDiff>0?"🔵":"🔴";
        await postChatMessage({type:"system",subtype:"leadChange",text:`🔄 Wende! ${leaderDot} ${leader} übernimmt in ${matchName} nach Loch ${holeNum}`,ts:Date.now()},`lead-${matchId}-${hi}`);
      } else if(prevRoundDiff!==0&&newRoundDiff===0&&!rs.won){
        await postChatMessage({type:"system",subtype:"equalize",text:`⚡ All Square! ${matchName} wieder ausgeglichen nach Loch ${holeNum}`,ts:Date.now()},`eq-${matchId}-${hi}`);
      }
    ...
    if(newStats.t1Confirmed>=newStats.needed||newStats.t2Confirmed>=newStats.needed){
      ...
      if(winKey!==lastPushKey){
        const wTeam=newStats.t1Confirmed>=newStats.needed?"t1":"t2";
        const {title,body}=buildTournamentWinMessage(winner,wPts,lPts,t1Name,t2Name);
        await sendPushToAll(title,body,winKey,wTeam);
        // v3.07: post tournament win to chat
        const dot=wTeam==="t1"?"🔵":"🔴";
        await postChatMessage({type:"system",subtype:wTeam==="t1"?"matchDecisionT1":"matchDecisionT2",text:`🏆🏆🏆 ${dot} ${winner} gewinnt den Ryder Cup!\n\nEndstand: ${wPts} – ${lPts}\n${winner} triumphiert über ${loser}!`,ts:Date.now()},winKey);
      }
    }
    setModal(null);
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.cream,fontFamily:"Georgia,serif"}}>
      ...
      <Header title="Ryder Cup" onBack={onBack} backLabel={backLabel||"Ausloggen"} theme={theme} onThemeToggle={onThemeToggle} T={T}
      ...
      <div style={{padding:"14px",maxWidth:"480px",margin:"0 auto"}}>
        ...
        {/* GESAMTPUNKTESTAND — hidden when chat tab is active (v3.10) */}
        {activeTab!=="chat"&&<div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:"12px",padding:"14px",marginBottom:"14px"}}>
        ...
        {activeTab==="chat"&&(()=>{
          // v3.15: Build match players list for player picker
          let matchPlayers=null;
          if(!isAdmin&&!isViewer&&role){
            const idx=parseInt(role.split("-")[1]);
            const allMatches=[];days.forEach(d=>d.matches.forEach(m=>allMatches.push(m)));
            const myMatch=allMatches[idx];
            if(myMatch){
              const ps=[];
              (myMatch.t1Pair||[]).forEach((name,i)=>{const pid=(myMatch.t1PlayerIds||[])[i];ps.push({id:pid,name,photo:(myMatch.t1Photos||{})[pid]||null,team:"t1"});});
              (myMatch.t2Pair||[]).forEach((name,i)=>{const pid=(myMatch.t2PlayerIds||[])[i];ps.push({id:pid,name,photo:(myMatch.t2Photos||{})[pid]||null,team:"t2"});});
              matchPlayers=ps;
            }
          }
          return <LiveChat role={role} T={T} mini={false} onClearChat={isAdmin?clearChat:null} matchPlayers={matchPlayers}/>;
        })()}
        {halftimePopup&&(
        ...
        {activeDay&&course&&(
          <>
            ...
            {(()=>{
              ...
              // v3.10: when day is done, show all matches in natural order (not pinned)
              const dayDone=activeDay.status==="done";
              const sorted=dayDone?activeDay.matches:[...activeDay.matches].sort((a,b)=>{
              ...
              return sorted.map((m,mi)=>{
                ...
                return(
                  <div key={m.id} ref={ref} style={{scrollMarginTop:"76px"}}>
                    <MatchCard match={m} pars={course.par} t1Name={t1Name} t2Name={t2Name} canEdit={canEdit(m.id,activeDay.id)} isAdmin={isAdmin} isViewer={isViewer} dayDone={dayDone} isOwnMatch={myMatchIds.includes(m.id)} T={T} captainT1={config.captainT1} captainT2={config.captainT2} onHoleClick={(matchId,hi)=>{
...
export default function App(){
  ...
  const [fanTeam,setFanTeam]=useState(()=>{try{return localStorage.getItem(FAN_TEAM_KEY);}catch(e){return null;}});
  const [showFanSelect,setShowFanSelect]=useState(false);
  const goToLogin=()=>{
  ...
  // v3.13: show fan selection for viewers who haven't picked a team
  if(phase==="waiting"&&!fanTeam)return(<>{logoutPopup}
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column"}}>
      <Header title="Wähle dein Team" onBack={handleLogout} backLabel="Ausloggen" theme={theme} onThemeToggle={toggleTheme} T={T}/>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",gap:"20px"}}>
        <div style={{fontSize:"13px",color:T.muted,textAlign:"center",marginBottom:"8px"}}>Welches Team unterstützt du als Fan?</div>
        <div style={{display:"flex",gap:"12px",width:"100%",maxWidth:"380px"}}>
          {["t1","t2"].map(team=>{
            const color=team==="t1"?T.blue:T.red;
            const name=team==="t1"?(config&&config.t1Name||"Team Europa"):(config&&config.t2Name||"Team USA");
            const players=team==="t1"?(config&&config.t1Players||[]):(config&&config.t2Players||[]);
            const dot=team==="t1"?"🔵":"🔴";
            return(
              <button key={team} onClick={()=>{
                setFanTeam(team);
                try{localStorage.setItem(FAN_TEAM_KEY,team);}catch(e){}
                try{const fuid="fan_"+(localStorage.getItem("ryder_uid")||"anon");setDoc(doc(db,"fanVotes",fuid),{team,ts:Date.now()});}catch(e){}
              }} style={{flex:1,background:T.surface,border:`2px solid ${color}44`,borderRadius:"12px",padding:"16px 10px",cursor:"pointer",textAlign:"center"}}>
                <div style={{fontSize:"24px",marginBottom:"6px"}}>{dot}</div>
                <div style={{fontSize:"14px",fontWeight:"700",color,marginBottom:"10px"}}>{name}</div>
                <div style={{display:"flex",flexDirection:"column",gap:"4px"}}>
                  {players.slice(0,6).map((p,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:"6px",justifyContent:"center"}}>
                      {p.photo?<img src={p.photo} style={{width:"20px",height:"20px",borderRadius:"50%",objectFit:"cover"}} alt=""/>:<div style={{width:"20px",height:"20px",borderRadius:"50%",background:color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"8px",color}}>{(p.fn||"")[0]}{(p.ln||"")[0]}</div>}
                      <span style={{fontSize:"10px",color:T.cream}}>{p.fn} {(p.ln||"")[0]}.</span>
                    </div>
                  ))}
                  {players.length>6&&<div style={{fontSize:"9px",color:T.faint}}>+{players.length-6} weitere</div>}
                </div>
              </button>
            );
          })}
  ...
  if(phase==="adminMenu"){
    ...
    if(!adminSection)return(<>{logoutPopup}<AdminMenu config={config} onSelect={handleSelectSection} onBack={handleLogout} theme={theme} onThemeToggle={toggleTheme} T={T}/></>);
    ...
    if(adminSection==="game"){
      if(!config)return(<>{logoutPopup}<AdminMenu config={config} onSelect={handleSelectSection} onBack={handleLogout} theme={theme} onThemeToggle={toggleTheme} T={T}/></>);
      return(<Dashboard config={config} role="admin" onBack={goToAdminMenu} backLabel="Menü" onEndTournament={handleEndTournament} theme={theme} onThemeToggle={toggleTheme}/>);
    }
  }
