...
function AdminSetup({ onStart }) {
  ...
  const buildMatches = (players1, players2) => {
    const matches = [];
    const count = Math.min(Math.floor(players1.length/2), Math.floor(players2.length/2), 4);
    for (let i=0; i<count; i++) {
      matches.push({ id:i, name:`Match ${i+1}`, pin:`MATCH${i+1}`, t1Pair:[players1[i*2]||"",players1[i*2+1]||""], t2Pair:[players2[i*2]||"",players2[i*2+1]||""], scores:emptyScores() });
    }
    return matches;
  };
  ...
  const start = async () => {
    setSaving(true);
    const day1Matches = buildMatches(t1Players, t2Players);
    const day2Matches = day1Matches.map((m,i)=>({ ...m, id:i+4, name:`Match ${i+5}`, pin:`MATCH${i+5}`, scores:emptyScores() }));
    const days = [
      { id:0, label:"Tag 1 – Riedhof", courseKey:"riedhof", matches:day1Matches },
      { id:1, label:"Tag 2 – Bergkramerhof", courseKey:"bergkramerhof", matches:day2Matches },
    ];
  ...
  const matchPreview = buildMatches(t1Players, t2Players);
    ...
    <div style={S.app}>
      ...
      <div style={S.body}>
        ...
        <div style={{ ...S.card, background:C.surface }}>
          <div style={{ fontSize:"11px", color:C.gold, fontWeight:"700", letterSpacing:"1px", marginBottom:"10px" }}>📋 MATCH-PINS (zum Verteilen)</div>
          ...
          <div style={{ fontSize:"10px", color:C.faint, marginTop:"8px" }}>
            Tag 2 bekommt MATCH5–MATCH8 (selbe Paarungen, neuer Platz)
          </div>
        </div>
  ...
  return (
    <div style={{ ...S.matchCard, borderColor: canEdit ? C.gold+"88" : C.border }}>
      ...
      <div style={{ padding:"10px 14px" }}>
        ...
        {!canEdit && (
          <div style={{ marginTop:"8px", fontSize:"10px", color:C.faint, textAlign:"center" }}>🔒 Nur lesbar – kein Zugriff auf dieses Match</div>
        )}
      </div>
    </div>
  );
...
function Dashboard({ config, role }) {
  ...
  // Which match IDs can this user edit?
  const editableMatchIds = isAdmin
    ? "all"
    : isViewer
    ? []
    : (() => {
        // role = "match-N", N is index across all matches globally
        const idx = parseInt(role.split("-")[1]);
        // collect all match ids in order
        const all = [];
        days.forEach(d => d.matches.forEach(m => all.push(m.id)));
        // player can edit match at position idx (day1) and idx+matchesPerDay (day2)
        const perDay = days[0]?.matches.length || 4;
        return [all[idx], all[idx + perDay]].filter(id => id !== undefined);
      })();

  const canEdit = (matchId) =>
    editableMatchIds === "all" || editableMatchIds.includes(matchId);
  ...
  if (phase==="waiting") return (
    <div style={{ ...S.app, display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh", padding:"20px" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <div style={{ color:C.muted, fontSize:"13px" }}>Der Admin muss zuerst das Turnier einrichten.</div>
        <RoleBadge role={role} />
      </div>
    </div>
  );
