...
async function sendPushToAll(title,body,dedupeKey,winnerTeam,senderUid) {
  if(dedupeKey&&dedupeKey===lastPushKey)return; lastPushKey=dedupeKey;
  try{await fetch('/api/send-push',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,body,tag:dedupeKey||'ryder-push',winnerTeam:winnerTeam||null,senderUid:senderUid||null})});}catch(e){}
}
...
function LiveChat({role,T,mini=true,onExpand,onClearChat}){
  ...
  const send=async()=>{
    ...
    const msgData={type:"user",author:p.name,photo:p.photo||null,authorId:myUid,text:text.trim()};
    ...
    try{await sendPushToAll(`💬 ${p.name}`,text.trim().slice(0,100),`chat-${Date.now()}`,null,myUid);}catch(e){}
  ...
  const sendPhoto=async(file)=>{
    ...
    try{
      ...
      try{await sendPushToAll(`📸 ${p.name}`,"Hat ein Foto geteilt",`photo-${Date.now()}`,null,myUid);}catch(e){}
