...
function useFanVotes(){
  ...
  useEffect(()=>{
    ...
    const u=onSnapshot(q,snap=>{let t1=0,t2=0;snap.forEach(d=>{const v=d.data();if(v.team==="t1")t1++;else if(v.team==="t2")t2++;});setVotes({t1,t2});});
    return()=>u();
  },[]);
  return votes;
}
function LiveChat({role,T,mini=true,onExpand,onClearChat,fanTeam}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"8px",height:"calc(100vh - 132px)"}}>
      ...
      {/* v3.13: Fan barometer */}
      {!mini&&!showSettings&&(()=>{
        const fv=fanVotesData;
        const total=fv.t1+fv.t2;
        if(total===0)return null;
        const pct=Math.round(fv.t1/total*100);
        return(
          <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"8px",padding:"8px 12px",marginBottom:"6px",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:"9px",color:T.faint,marginBottom:"4px"}}>
              <span style={{color:T.blue}}>🔵 {fv.t1} Fans</span>
              <span style={{fontWeight:"700",color:T.gold}}>Fan-Barometer</span>
              <span style={{color:T.red}}>🔴 {fv.t2} Fans</span>
            </div>
            <div style={{display:"flex",height:"8px",borderRadius:"4px",overflow:"hidden",background:T.elevated}}>
              <div style={{width:pct+"%",background:T.blue,transition:"width 1s cubic-bezier(0.4,0,0.2,1)",borderRadius:"4px 0 0 4px"}}/>
              <div style={{width:(100-pct)+"%",background:T.red,transition:"width 1s cubic-bezier(0.4,0,0.2,1)",borderRadius:"0 4px 4px 0"}}/>
            </div>
          </div>
        );
      })()}
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.cream,fontFamily:"Georgia,serif"}}>
      ...
      <div style={{padding:"14px",maxWidth:"480px",margin:"0 auto"}}>
        ...
        {activeTab==="chat"&&<LiveChat role={role} T={T} mini={false} fanTeam={fanTeam} onClearChat={isAdmin?clearChat:null}/>}
        {halftimePopup&&(
          ...
          {["t1","t2"].map(team=>{
            ...
            return(
              <button key={team} onClick={()=>{
                ...
                try{setDoc(doc(db,"fanVotes",myPushUid||"anon"),{team,ts:Date.now()});}catch(e){}
