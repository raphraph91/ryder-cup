...
function LiveChat({role,T,mini=true,onExpand,onClearChat}){
  ...
  const send=async()=>{
    ...
    // v3.08: push notification for user messages
    try{await sendPushToAll(`💬 ${p.name}`,text.trim().slice(0,100),`chat-${Date.now()}`);}catch(e){}
  ...
  const sendPhoto=async(file)=>{
    ...
    try{
      ...
      // v3.08: push notification for photo messages
      try{await sendPushToAll(`📸 ${p.name}`,"Hat ein Foto geteilt",`photo-${Date.now()}`);}catch(e){}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      <div ref={scrollRef} onScroll={handleScroll} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"10px 12px",height:"420px",overflowY:"auto",WebkitOverflowScrolling:"touch",display:"flex",flexDirection:"column",justifyContent:visible.length<6?"flex-end":"flex-start",position:"relative"}}>
        {visible.length===0&&(
    ...
    <div style={{minHeight:"100vh",background:"#0D2B1A",color:"#F2EDD7",fontFamily:"Georgia,serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,padding:"20px 20px 40px",maxWidth:"400px",width:"100%",margin:"0 auto",display:"flex",flexDirection:"column",justifyContent:"center"}}>
        ...
        <div style={{background:"#0F2D1A",border:"1px solid #2D6B40",borderRadius:"8px",padding:"12px",textAlign:"left"}}>
          ...
          {[["👑 Admin","••••••••"],["👁 Zuschauer","••••••••"],["⛳ Spieler","MATCH1 – 8"]].map(([r,c])=>(
...
function Dashboard({config,role,onBack,onEndTournament,theme,onThemeToggle,onRefreshConfig}){
  ...
  useEffect(()=>{if(!messaging)return;const u=onMessage(messaging,p=>{const title=p.data?.title||"";if(title.indexOf("🏆")!==-1||title.indexOf("gewinn")!==-1)return;setToast({title,body:p.data?.body});});return()=>u();},[]);
