...
function Login({onLogin,theme,onThemeToggle}){
  ...
  return(
    <div style={{minHeight:"100vh",background:isDark?"#0D2B1A":T.bg,color:T.cream,fontFamily:"Georgia,serif",display:"flex",flexDirection:"column"}}>
      <div style={{position:"fixed",top:"16px",right:"16px",zIndex:20}}><ThemeToggle theme={theme} onToggle={onThemeToggle}/></div>

      {/* Photo hero — always dark overlay regardless of theme */}
      <div style={{position:"relative",height:"52vh",minHeight:"220px",overflow:"hidden",flexShrink:0}}>
        ...
        <div style={{position:"absolute",inset:0,background:isDark
          ?"linear-gradient(180deg,rgba(7,26,14,0.1) 0%,rgba(7,26,14,0.55) 55%,rgba(7,26,14,0.97) 100%)"
          :"linear-gradient(180deg,rgba(0,0,0,0.05) 0%,rgba(0,0,0,0.45) 55%,rgba(245,248,245,0.97) 100%)"
        }}/>
        <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"18px 20px 20px"}}>
          ...
          {/* Fix 1: 2026 */}
          <div style={{fontSize:"11px",color:isDark?"rgba(255,255,255,0.5)":"rgba(0,0,0,0.55)",letterSpacing:"3px",marginTop:"5px",textTransform:"uppercase"}}>2026 · Friends Edition</div>
          {/* Fix 3: hosted by */}
          <div style={{fontSize:"10px",color:isDark?"rgba(255,255,255,0.35)":"rgba(0,0,0,0.4)",letterSpacing:"1px",marginTop:"6px",fontStyle:"italic"}}>hosted by Niklas Lehmann &amp; Nicolai Leppla</div>
        </div>
      </div>

      {/* Login form — Fix 4: uses T colors for day/night */}
      <div style={{flex:1,padding:"20px 20px 40px",maxWidth:"400px",width:"100%",margin:"0 auto",display:"flex",flexDirection:"column",justifyContent:"center"}}>
        <input
          style={{width:"100%",padding:"14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:"8px",color:T.cream,fontSize:"20px",letterSpacing:"6px",textAlign:"center",marginBottom:"10px",boxSizing:"border-box",outline:"none"}}
        ...
        <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"8px",padding:"12px",textAlign:"left"}}>
        ...
        <div style={{textAlign:"center",fontSize:"10px",color:T.faint,marginTop:"12px",letterSpacing:"1px"}}>v{VERSION}</div>
      </div>
    </div>
  );
}
    ...
    return(
      <div style={{marginTop:"10px",padding:"10px",background:bgColor+"44",borderRadius:"10px",border:`1px solid ${color}33`}}>
          ...
          {shots.map((pid,i)=>{
            ...
            return(
              <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"2px"}}>
                <div style={{fontSize:"7px",color:T.faint,textAlign:"center"}}>{i+1}{isPutt?"⛳":""}</div>
                <button onClick={()=>toggleShot(playerIds===t1PlayerIds?"t1":"t2",i)}
                  style={{width:"38px",height:"38px",borderRadius:"50%",border:`2px solid ${isPutt?"#2E7D32":color}`,background:isPutt?T.isDark?"#0A2014":"#E8F5E9":bgColor+"88",cursor:"pointer",overflow:"hidden",padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {photo
                    ?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt={name}/>
                    :<span style={{fontSize:"10px",fontWeight:"900",color:isPutt?"#2E7D32":color}}>{(pid||"?").slice(0,2)}</span>}
                </button>
              </div>
            );
          })}
        ...
        <div style={{display:"flex",alignItems:"center",gap:"8px",background:T.isDark?"#060E1A":T.elevated,borderRadius:"8px",padding:"6px 10px",border:`0.5px solid ${T.border}`}}>
          ...
          <button onClick={()=>setPutts(p=>Math.max(1,p-1))} style={{width:"26px",height:"26px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"5px",fontSize:"14px",cursor:"pointer",color:T.muted}}>−</button>
          <span style={{fontSize:"16px",fontWeight:"900",color:"#2E7D32",width:"20px",textAlign:"center"}}>{putts}</span>
          <button onClick={()=>setPutts(p=>Math.min(score-1,p+1))} style={{width:"26px",height:"26px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"5px",fontSize:"14px",cursor:"pointer",color:T.muted}}>+</button>
          <span style={{fontSize:"9px",fontWeight:"700",color:"#2E7D32",background:T.isDark?"#0A2014":"#E8F5E9",borderRadius:"4px",padding:"1px 6px"}}>{PUTT_LABELS[putts]||putts+" Putts"}</span>
        </div>
        {/* Summary line */}
        <div style={{marginTop:"6px",fontSize:"9px",color:T.faint}}>
          {playerIds.map((pid,i)=>{
            const cnt=shots.filter(s=>s===pid).length;
            const puttCnt=shots.slice(score-putts).filter(s=>s===pid).length;
            const name=(pair||[])[i]||pid;
            const [fn]=splitName(name);
            return <span key={pid} style={{marginRight:"8px",color,fontWeight:"600"}}>{fn}: {cnt} Schl. ({puttCnt} Putt)</span>;
          })}
        </div>
      </div>
    );
              ...
              {(()=>{
                ...
                return(
                  <div style={{marginTop:"10px",padding:"10px",background:T.red+"22",borderRadius:"10px",border:`1px solid ${T.red}33`}}>
                        ...
                        return(
                          <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"2px"}}>
                            ...
                            <button onClick={()=>toggleShot("t2",i)}
                              style={{width:"38px",height:"38px",borderRadius:"50%",border:`2px solid ${isPutt?"#2E7D32":T.red}`,background:isPutt?T.isDark?"#0A2014":"#E8F5E9":T.red+"22",cursor:"pointer",overflow:"hidden",padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                              {photo
                                ?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt={name}/>
                                :<span style={{fontSize:"10px",fontWeight:"900",color:isPutt?"#2E7D32":T.red}}>{(pid||"?").slice(0,2)}</span>}
                            </button>
                          </div>
                        );
                      })}
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:"8px",background:T.isDark?"#060E1A":T.elevated,borderRadius:"8px",padding:"6px 10px",border:`0.5px solid ${T.border}`}}>
                      <span style={{fontSize:"9px",color:T.faint,flex:1}}>⛳ Davon Putts:</span>
                      <button onClick={()=>setT2Putts(p=>Math.max(1,p-1))} style={{width:"26px",height:"26px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"5px",fontSize:"14px",cursor:"pointer",color:T.muted}}>−</button>
                      <span style={{fontSize:"16px",fontWeight:"900",color:"#2E7D32",width:"20px",textAlign:"center"}}>{t2Putts}</span>
                      <button onClick={()=>setT2Putts(p=>Math.min(t2-1,p+1))} style={{width:"26px",height:"26px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"5px",fontSize:"14px",cursor:"pointer",color:T.muted}}>+</button>
                      <span style={{fontSize:"9px",fontWeight:"700",color:"#2E7D32",background:T.isDark?"#0A2014":"#E8F5E9",borderRadius:"4px",padding:"1px 6px"}}>{{1:"1-Putt ⚡",2:"2-Putt ✓",3:"3-Putt ⚠️"}[t2Putts]||t2Putts+" Putts"}</span>
                    </div>
                    <div style={{marginTop:"6px",fontSize:"9px",color:T.faint}}>
                      {t2PlayerIds.map((pid,i)=>{
