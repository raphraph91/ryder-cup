...
function ScoreModal({match,holeIndex,t1Name,t2Name,existing,par,onSave,onClose,T}){
  ...
  const ShotSequenceBlock=({score,shots,setShots,putts,setPutts,playerIds,photos,pair,color,bgColor})=>{
    ...
    return(
      <div style={{marginTop:"10px",padding:"10px",background:bgColor+"44",borderRadius:"10px",border:`1px solid ${color}33`}}>
        <div style={{fontSize:"9px",color,fontWeight:"700",letterSpacing:"1px",marginBottom:"8px"}}>🏌️ SCHLAG-SEQUENZ</div>
        {/* Player legend */}
          ...
          {playerIds.map((pid,i)=>{
            ...
            return(
              <div key={pid} style={{display:"flex",alignItems:"center",gap:"4px"}}>
                <PlayerAvatar name={name} size={22} color={color} photo={photo}/>
                <span style={{fontSize:"9px",color,fontWeight:"600"}}>{fn}</span>
              </div>
            );
          })}
        ...
        <div style={{display:"flex",alignItems:"flex-end",flexWrap:"wrap",gap:"4px",marginBottom:"10px"}}>
          {shots.map((pid,i)=>{
            ...
            // Type symbol shown above slot
            const typeSymbol=isPutt?"⛳":isAbschlag?"🏌️":"🟢";
            const typeLabel=isPutt?"P":isAbschlag?"A":"F";
            return(
        ...
        {/* Putt counter moved inline next to score stepper (Fix 5) */}
        {/* Summary line with shot type breakdown */}
        <div style={{marginTop:"6px",fontSize:"9px",color:T.faint}}>
          {playerIds.map((pid,i)=>{
            ...
            const name=(pair||[])[i]||pid;
            const [fn]=splitName(name);
            return <span key={pid} style={{marginRight:"8px",color,fontWeight:"600"}}>{fn}: 🏌️{abIdx} 🟢{fwIdx} ⛳{puttIdx}</span>;
          })}
        </div>
      </div>
    );
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:100}} onClick={onClose}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"16px 16px 0 0",padding:"16px 18px 30px",width:"100%",maxWidth:"480px",maxHeight:"92vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"12px",marginBottom:"14px"}}>
            ...
            <div style={{background:T.blue+"11",borderRadius:"12px",border:`1px solid ${T.blue}33`,padding:"10px 12px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px"}}>
                ...
                {/* Fix 3: Putt counter — fixed position (label on top), never shifts */}
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginLeft:"auto"}}>
                  <div style={{fontSize:"8px",fontWeight:"700",color:"#2E7D32",marginBottom:"3px",height:"14px",lineHeight:"14px",whiteSpace:"nowrap"}}>⛳ {{0:"Chip-in",1:"1-Putt⚡",2:"2-Putt✓",3:"3-Putt⚠️"}[t1Putts]||t1Putts+"P"}</div>
                  <div style={{display:"flex",alignItems:"center",gap:"5px",background:T.isDark?"#060E1A":T.elevated,borderRadius:"8px",padding:"5px 8px",border:`0.5px solid ${T.border}`}}>
                    <button onClick={()=>setT1Putts(p=>Math.max(0,p-1))} style={{width:"22px",height:"22px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"4px",fontSize:"13px",cursor:"pointer",color:T.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                    <span style={{fontSize:"14px",fontWeight:"900",color:"#2E7D32",width:"16px",textAlign:"center"}}>{t1Putts}</span>
                    <button onClick={()=>setT1Putts(p=>Math.min(t1,p+1))} style={{width:"22px",height:"22px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"4px",fontSize:"13px",cursor:"pointer",color:T.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                  </div>
                </div>
              </div>
              {t1ShotsBlock}
            ...
            <div style={{background:T.red+"11",borderRadius:"12px",border:`1px solid ${T.red}33`,padding:"10px 12px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px"}}>
                ...
                {/* Fix 3: Putt counter — fixed position (label on top), never shifts */}
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginLeft:"auto"}}>
                  <div style={{fontSize:"8px",fontWeight:"700",color:"#2E7D32",marginBottom:"3px",height:"14px",lineHeight:"14px",whiteSpace:"nowrap"}}>⛳ {{0:"Chip-in",1:"1-Putt⚡",2:"2-Putt✓",3:"3-Putt⚠️"}[t2Putts]||t2Putts+"P"}</div>
                  <div style={{display:"flex",alignItems:"center",gap:"5px",background:T.isDark?"#060E1A":T.elevated,borderRadius:"8px",padding:"5px 8px",border:`0.5px solid ${T.border}`}}>
                    <button onClick={()=>setT2Putts(p=>Math.max(0,p-1))} style={{width:"22px",height:"22px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"4px",fontSize:"13px",cursor:"pointer",color:T.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                    <span style={{fontSize:"14px",fontWeight:"900",color:"#2E7D32",width:"16px",textAlign:"center"}}>{t2Putts}</span>
                    <button onClick={()=>setT2Putts(p=>Math.min(t2,p+1))} style={{width:"22px",height:"22px",border:`1px solid ${T.border}`,background:T.elevated,borderRadius:"4px",fontSize:"13px",cursor:"pointer",color:T.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                  </div>
                </div>
              </div>
        ...
        <button
          ...
          style={{width:"100%",padding:"14px",background:scrambleFilled?`linear-gradient(135deg,${T.gold},#A07830)`:T.elevated,border:"none",borderRadius:"8px",color:scrambleFilled?T.isDark?"#0D2B1A":"white":T.muted,fontSize:"15px",fontWeight:"900",letterSpacing:"2px",cursor:scrambleFilled?"pointer":"not-allowed",opacity:scrambleFilled?1:0.5}}
          onClick={handleSave}>
          Speichern
        </button>
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.cream,fontFamily:"Georgia,serif"}}>
      ...
      <Header title="Ryder Cup" onBack={onBack} backLabel="Menü" theme={theme} onThemeToggle={onThemeToggle} T={T}
        rightSlot={<div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          <button onClick={async()=>{setSaving(true);try{const s=await getDoc(doc(db,"tournaments","ryder2024"));if(s.exists()&&onRefreshConfig)onRefreshConfig(s.data());}catch(e){}setSaving(false);}} style={{background:"transparent",border:`1px solid rgba(255,255,255,0.2)`,borderRadius:"6px",padding:"4px 8px",color:"rgba(255,255,255,0.6)",fontSize:"11px",cursor:"pointer",display:"flex",alignItems:"center",gap:"4px"}}>
            🔄
          </button>
