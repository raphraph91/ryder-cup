...
function HoleHeatmap({holeWins,T}){
  // Fix 2 (v3.05): Stacked bars showing how many of the 4 matches each team
  // won per hole. E.g. Loch 1: 3x Europa, 1x USA → 75% blue, 25% red.
  const BAR_H=32;
  return(
    <div>
      {[0,1].map(r=>{
        const holes=Array.from({length:9},(_,i)=>{
          const hi=r*9+i;
          return{hi,t1:holeWins.t1[hi]||0,t2:holeWins.t2[hi]||0,tie:holeWins.tie[hi]||0};
        });
        return(
          <div key={r} style={{marginBottom:12}}>
            <div style={{fontSize:8,color:T.faint,letterSpacing:1,marginBottom:4,fontWeight:700}}>{r===0?"Runde 1 · Loch 1–9":"Runde 2 · Loch 10–18"}</div>
            <div style={{display:"flex",gap:3,marginBottom:3}}>
              {holes.map(h=><div key={h.hi} style={{flex:1,textAlign:"center",fontSize:8,color:T.faint}}>{h.hi+1}</div>)}
            </div>
            <div style={{display:"flex",gap:3}}>
              {holes.map(h=>{
                const total=h.t1+h.t2+h.tie;
                if(total===0)return(<div key={h.hi} style={{flex:1,height:BAR_H,background:T.elevated,borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:T.faint}}>·</div>);
                return(
                  <div key={h.hi} style={{flex:1,height:BAR_H,display:"flex",flexDirection:"column",borderRadius:4,overflow:"hidden",border:`1px solid ${T.border}`}} title={"EU "+h.t1+" / Tie "+h.tie+" / US "+h.t2}>
                    {h.t1>0&&<div style={{flex:h.t1,background:T.blue,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:9,color:"#fff",fontWeight:700}}>{h.t1}</span></div>}
                    {h.tie>0&&<div style={{flex:h.tie,background:T.isDark?"#2A3A2A":"#E0E0E0",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:8,color:T.faint}}>{h.tie}</span></div>}
                    {h.t2>0&&<div style={{flex:h.t2,background:T.red,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:9,color:"#fff",fontWeight:700}}>{h.t2}</span></div>}
                  </div>
                );
              })}
            </div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginTop:8}}>
        <div style={{display:"flex",gap:6,alignItems:"center",fontSize:10}}>
          <div style={{width:12,height:8,borderRadius:2,background:T.blue}}/><span style={{color:T.blue}}>Europa</span>
          <div style={{width:12,height:8,borderRadius:2,background:T.isDark?"#2A3A2A":"#E0E0E0"}}/><span style={{color:T.faint}}>Geteilt</span>
          <div style={{width:12,height:8,borderRadius:2,background:T.red}}/><span style={{color:T.red}}>USA</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{display:"flex",gap:5,marginBottom:12}}>
        {["shots","abschlaege","fairways","putts"].map(k=>(
          <button key={k} onClick={()=>setContribFilter(k)}
            style={{flex:1,padding:"7px 4px",borderRadius:8,border:`1px solid ${contribFilter===k?T.gold:T.border}`,background:contribFilter===k?"#0D2B1A":"transparent",color:contribFilter===k?T.gold:T.muted,fontSize:10,fontWeight:contribFilter===k?700:400,cursor:"pointer",textAlign:"center"}}>
            {filterLabels[k]}
          </button>
        ))}
      </div>
